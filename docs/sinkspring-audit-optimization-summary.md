...
- 验证前去重已开始落地：确定性候选与模型候选在漏洞类型、规范化 Sink 锚点和最近路径节点一致时合并，保留确定性候选 ID，并合并证据、补证需求与来源；不同 Mapper statement 仍由最近路径节点隔离。
- 模型候选解析出的真实入口和 Source 会回填确定性候选，不再保留“待由调用者闭包解析”的占位信息。
- 路径验证结果新增可选的 `resolved_entry`、`resolved_source`、`resolved_hops`、`resolved_sink` 和 `resolved_controls`，Finding Builder 优先使用这些已解析字段生成正式发现。
- 已增加专项回归测试；随后完成的 Docker 重跑结果记录在下一节，上方历史阶段数据保持原值以便纵向对照。

### 2026-09-09 Docker 重跑结果

使用用户提供的 schema v2 标准数据在审计结束后离线评分，标准答案未进入模型上下文。当前任务 `87c69ad6000d480c8e685c56fbd6ecb2` 严格命中 28/35，召回率 80.00%，代码召回率 86.67%，配置召回率 40.00%，7 个安全控制误报为 0。对同一标准重评上次任务 `6e253f09ae7d40e2b599c8a492b4ed4c` 为 17/35（48.57%）。

路径草图从 121 降至 92，但 55 条 supported 路径产生 47 条正式发现，标准项内仍有 20 条重复报告；有效续跑使用 151 次调用和 1,673,058 Token，未达到成本目标。漏报为 V21、V24、V25、V30、Cfg02、Cfg04、Cfg05。完整对照见 `reports/sinkspring-comparison-87c69ad6.md`。
...
## 2026-09-09 全量改造后的 Docker 验证

最终任务 `1f29fd6455c54bc29f8b23d08c261396` 使用同一 SinkSpring 快照与标准数据完成报告组装：
...
本轮实现了路径草图去重、解析路径字段、按路径角色分配证据槽、续读截断证据、配置确定性验证、最终 Finding 去重和 Scope Worker 单次新上下文重试。验证中额外修复了两个只在真实续跑暴露的问题：正式证据字段 `id` 的兼容，以及未初始化 `model_calls` 的安全读取。最终续跑复用了 59 条独立复核，只新增 1 次模型调用即完成报告。

当前仍漏报 V21、V24、V25、V30。评估器显示 5 个重复匹配，但逐项核对为不同语义缺陷（两种反序列化汇点、不同硬编码秘密、不同配置键）；没有为追求指标而错误合并。

## 相关报告

- `reports/sinkspring-comparison-25824ee8.md`
- `reports/sinkspring-comparison-87c69ad6.md`
